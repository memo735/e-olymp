...
int i, n, m, L, R, pos, type, Value, mas[50001];
...
//Update изменяет значение элемента с индексом pos на Value.
void Update(int v, int LeftPos, int RightPos, int pos, int Value)
{
	if (LeftPos == RightPos)
	{
		SegTree[v].max = SegTree[v].prefix = SegTree[v].suffix = SegTree[v].sum = Value;
		return;
	}
	int Middle = (LeftPos + RightPos) / 2;
